...
class BtlParserTest implements TestCase {
  ...
  it_has_if_statements() async {
    expect(await parser.parse(r'<if $show>shown</if>', {'show': true}), equals('shown'));
    expect(await parser.parse(r'<if $show>shown</if>', {'show': false}), equals(''));
  }
...
<for each=$item in=$items>
  ...
  @test
  it_can_access_object_fields_as_nested_variables() async {
    var template = r'<div>${test.property}</div>';
    expect(await parser.parse(template, {'test': new TestClass()}), equals('<div>value</div>'));
  }

  @test
  it_allows_for_complex_expressions_within_bracketed_variables() async {
    var template = r'''
    ${(1 + 1) * 2 / intVar}
    ...
    4
    ''';
    expect(await parser.parse(template, {
      'intVar': intVar,
      'boolVar': boolVar,
      'stringVar': stringVar
    }), equals(resultOne));
  ...
  @test
  it_can_have_global_functions_in_expression() async {
    expect(await parser.parse(r'${globalFunction("value")}'), equals('value plus'));
  }
}
